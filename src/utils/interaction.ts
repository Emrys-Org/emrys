...
import { DOGE_DECIMALS, LTC_DECIMALS } from "./constant";
...
// Helper function to determine the asset decimals and asset name
const getAssetInfo = (
  cryptoType: CryptoCurrency = CryptoCurrency.BTC
): { decimals: number; assetName: string } => {
  switch (cryptoType) {
    case CryptoCurrency.DOGE:
      return { decimals: DOGE_DECIMALS, assetName: "ZDOGE" };
    case CryptoCurrency.LTC:
      return { decimals: LTC_DECIMALS, assetName: "ZLTC" };
    case CryptoCurrency.BTC:
    default:
      return { decimals: BTC_DECIMALS, assetName: "ZBTC" };
  }
};
...
export const getDepositAssetTo = (
  ...
  const { decimals, assetName } = getAssetInfo(cryptoType);
...
export const getWithdrawalAssetTo = (
  ...
  const { decimals } = getAssetInfo(cryptoType);
