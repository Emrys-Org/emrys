...
import { deriveBitcoinWallet, getBitcoinConnectorWallet, txConfirm } from '../bitcoin/wallet';
...
export interface BitcoinWalletContextState {
  ...
  connectConnectorWallet: (connector: BaseConnector, isReconnect?: boolean) => Promise<void>;
...
const BitcoinWalletContext = createContext<BitcoinWalletContextState | null>(null);
...
export function BitcoinWalletProvider({ children }: { children: ReactNode }) {
  ...
  const { publicKey: solanaPubkey, signMessage: solanaSignMessage } = useWallet();
  const [bitcoinWallet, setBitcoinWallet] = useState<BitcoinWallet | null>(null);
  const [connecting, setConnecting] = useState(false);
  const [connected, setConnected] = useState(false);
  const [disconnecting, setDisconnecting] = useState(false);
  const [bitcoinWalletType, setBitcoinWalletType] = useState<BitcoinWalletType>(null);
  const [prevBitcoinNetwork, setPrevBitcoinNetwork] = useState<BitcoinNetwork | undefined>(
    bitcoinNetwork,
  );
  const [prevSolanaPubkey, setPrevSolanaPubkey] = useState<PublicKey | null | undefined>(undefined);
  ...
  const handleSignPsbt = useCallback(
    async (psbt: bitcoin.Psbt, tweaked?: boolean) => {
      ...
      if (bitcoinWalletType === 'solana' && bitcoinWallet.tweakSigner && bitcoinWallet.signer) {
        if (tweaked) {
  ...
  const handleDerivedWalletConnect = useCallback(async () => {
    ...
    try {
      ...
      if (solanaSignMessage) {
        ...
        res = await deriveBitcoinWallet(solanaPubkey, bitcoinNetwork, solanaSignMessage);
      }
