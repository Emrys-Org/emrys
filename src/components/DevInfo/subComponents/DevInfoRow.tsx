...
const DevInfoRow = ({
  ...
  if (withIcon) {
    return (
      <div key={value} className="text-shade-mute relative flex cursor-pointer gap-x-2">
        <div onClick={() => handleCopy(value)} className="hover:text-shade-secondary mt-0.5">
          <CopyIcon />
        </div>
        <Link href={getLink(value, linkType)} target="_blank" rel="noopener noreferrer">
