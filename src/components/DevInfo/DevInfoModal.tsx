...
import { BTC_DECIMALS, LINK_TYPE, MODAL_NAMES, SOLANA_TX_FEE_IN_SOL } from '@/utils/constant';
...
const getNetworkLabel = (networkType: BitcoinNetwork | SolanaNetwork, chain: Chain) => {
    ...
    >
      <div className="flex items-center justify-between pb-5 pl-2 pr-1">
        <div className="flex items-center space-x-2">
          <Icon name="DevInfo" />
          <div className="text-shade-secondary font-semibold">Dev Info</div>
        </div>
        <div>
          <div
            onClick={closeModal}
            className="hover:text-shade-primary relative h-[18px] w-[18px] cursor-pointer"
          >
            <Icon name="Close" />
          </div>
        </div>
      </div>
      <div className="border-shade-mute/15 bg-shade-background flex flex-col space-y-6 rounded-2xl border p-3">
        <Tabs
          tabs={tabs}
          activeTab={activeTab}
        ...
        >
          {devInfoList[activeTab].map((item, index) => (
            <div key={item.title}>
              {item.title && (
                <div className="text-shade-primary pb-2 text-lg font-semibold">{item.title}</div>
              )}
                ...
                <div className="flex flex-col gap-y-4">
                  {item.list.length === 1 ? (
                    ...
                    item.list.map((pda, pdaIndex) => (
                      <div key={pda.title}>
                        <>
                          <div className="text-shade-secondary pb-1 font-semibold">{pda.title}</div>
                          {
                            <DevInfoRow
                              value={pda.value ?? ''}
