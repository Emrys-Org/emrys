...
import { formatBitcoinAddress, formatSolanaAddress, formatValue } from '@/utils/format';
...
export default function WalletSelector() {
  ...
  const { data: bitcoinUTXOs, isLoading: isBitcoinUTXOsLoading } = useBitcoinUTXOs(
    bitcoinWallet?.p2tr,
  );
  const { data: zbtcBalance, isLoading: isZbtcBalanceLoading } = useBalance(solanaPubkey);
  const { data: positions, isLoading: isPositionsLoading } = usePositions(solanaPubkey);

  const [showSolanaMenu, setShowSolanaMenu] = useState(false);
  const [showBitcoinMenu, setShowBitcoinMenu] = useState(false);
  const [prevSolanaPubkey, setPrevSolanaPubkey] = useState<PublicKey | null>(solanaPubkey);

  const btcBalance =
    bitcoinUTXOs?.reduce((acc, cur) => acc.plus(cur.satoshis), new BigNumber(0)) ||
    new BigNumber(0);
      ...
      <div className={styles.walletSelector}>
        <div className={styles.walletSelector__header}>
          ...
          <div
            ...
            className="hover:text-shade-primary h-[18px] w-[18px] cursor-pointer"
          >
            <Icon name="Close" />
          </div>
        </div>
        <div className={styles.walletSelector__networks}>
          <div className={styles.walletSelector__networks__list}>
            {networks.map((network) => (
              ...
              >
                <div className={styles.walletSelector__networks__network__header}>
                  <div className={styles.walletSelector__networks__network__header__title}>
                    <Icon name={network.icon as IconName} size={18} />
                    <span>{network.title}</span>
                  </div>
                  <div className={styles.walletSelector__networks__network__header__status}>
                    {/* Solana Connect */}
                    {network.title === 'Solana' && (
                      <div
                        className={styles.walletSelector__networks__network__header__status__solana}
                        onClick={() => {
                      ...
                      >
                        <Button
                          ...
                          icon={
                            <Image
                              src={solanaWallet?.adapter.icon ?? '/icons/phantom.svg'}
                              alt="wallet icon"
                              width={16}
                              height={16}
                            />
                        ...
                        {showSolanaMenu && (
                          ...
                          >
                            <div className={styles.walletSelector__hoverMenu__content}>
                              <div
                                className={styles.walletSelector__hoverMenu__content__item}
                ...
                {solanaAssetList.map((asset, index) => (
                  <div key={index} className={`${index === 0 ? '!mt-2' : ''}`}>
                    <div className={styles.walletSelector__balances__asset}>
                      <div className={styles.walletSelector__balances__asset__info}>
                        <div className={styles.walletSelector__balances__asset__info__title}>
                          <Image
                          ...
                          />
                          <div
                            className={styles.walletSelector__balances__asset__info__title__text}
                          >
                            <span>{asset.title}</span>
                            {asset.type === 'custodian' && (
                              <div className={styles.walletSelector__balances__asset__info__type}>
                                <Icon name="Lock" />
                              </div>
                            )}
                          </div>
                      ...
                      >
                        ...
                        <span className="text-shade-mute text-sm">
                          {isPositionsLoading && isZbtcBalanceLoading ? (
                            <Skeleton width="100px" height="10px" />
                          ) : (
                            `~$${asset.value}`
                          ...
                          <div className={styles.walletSelector__balances__asset__info}>
                            <div className={styles.walletSelector__balances__asset__info__title}>
                              ...
                              >
                                ...
                                {asset.type === 'custodian' && (
                                  <div
                                    className={styles.walletSelector__balances__asset__info__type}
                                  >
                                    <Icon name="Lock" />
                                  </div>
                                )}
