...
function ReconnectModal() {
  ...
  const { publicKey: solanaPubkey, disconnect: disconnectSolanaWallet } = useWallet();
  const { setVisible: setShowSolanaModal } = useWalletModal();
  const { checkHotReserveBucketStatus } = useHotReserveBucketActions(bitcoinWallet);
    ...
    >
      <div className="relative">
        <div
          ...
          className="hover:text-shade-primary absolute right-0 top-0 h-[18px] w-[18px] cursor-pointer"
        >
          <Icon name="Close" />
        </div>
      </div>
      <div className="flex flex-col items-center">
        <div className="text-shade-primary max-w-[75%] text-center text-xl font-semibold">
          Solana Address Not Matched
        </div>
        <div className="mb-4 mt-4 text-center text-base font-medium">
          This Bitcoin account is already paired to the Solana wallet address shown below. Please
          switch to that address to continue:
        </div>
