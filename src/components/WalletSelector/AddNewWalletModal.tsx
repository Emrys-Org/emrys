...
export default function AddNewWalletModal() {
  ...
  const [walletDetection, setWalletDetection] = useState(checkWalletAvailability);
  ...
  const [selectedWallet, setSelectedWallet] = useState(
    getDefaultSelectedWallet(connector, regtestWallets, testnetWallets, bitcoinNetwork),
  );
  ...
  const onConnectorConnect = async (wallet: Wallet, connector: BaseConnector) => {
    ...
    try {
      ...
      if (walletError?.code === 4001) {
        notifyError('You must have at least one account in the Bitcoin wallet.');
      }
      setIsConnecting(false);
    }
  };
  ...
  const connectWallet = async (wallet: Wallet & { originalIndex: number }) => {
    if (wallet.type === 'connector') {
      const connector = connectors.find((item) => item.metadata.id === wallet.id);
    ...
    >
      ...
      <div className={styles.wallet__modal__walletList}>
        {wallets
          ...
          .sort((a, b) => (a.isDetected === b.isDetected ? 0 : a.isDetected ? -1 : 1))
          .map((wallet) => (
            <div
              key={wallet.title}
              className={styles.wallet__modal__walletList__item}
              onClick={() => {
                if (!wallet.isDetected) return window.open(wallet.url, '_blank');
            ...
            >
              <div className={styles.wallet__modal__walletList__item__icon}>
                <Image
                  src={wallet.type === 'solana' ? wallet.icon : `/icons/${wallet.icon}.svg`}
                  alt={wallet.title}
                  width={18}
                  height={18}
                />
              ...
              {isConnecting && selectedWallet === wallet.originalIndex && <ButtonLoader />}
              {!isConnecting && (
                <div className={`${styles.wallet__modal__walletList__item__status}`}>
